...
肤色与采集区间可能相关，几何模型容易把肤色当作脸型捷径。但强颜色增强又会破坏 `skin_color`、`eye_color` 和 `hair_color` 标签。

推荐两种实现之一：

1. 几何分支使用强亮度/颜色增强或随机灰度，颜色分支使用轻增强 RGB；
2. 第一阶段只训练脸型参数，第二阶段单独训练颜色预测。

离线 JPEG 保留原始 RGB，不提前灰度化，保证两种训练策略都可使用。
