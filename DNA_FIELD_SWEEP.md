...
4. Allele 留空或保留当前 allele，即只扫描强度；输入多个 allele，例如
   ...
   勾选“当前字段完成后，自动扫描所有后续字段”时，这个输入只作用于第一个
   字段；后续字段会自动读取各自在基础 DNA 中的 allele，不会错误沿用第一个
   字段的 allele 名称。
...
勾选“当前字段完成后，自动扫描所有后续字段”，工具会按照基础 DNA 中 gene
的原始顺序，从当前下拉框选中的字段一直扫描到最后一个字段。开始前会显示
字段数和变体总数并要求确认。

- 每个字段创建独立的 `session.json`、manifest、DNA 和截图目录；
- 同一批目录使用相同时间戳和三位字段序号，例如
  `20260804_220000_001_gene_chin_forward`；
- 输出根目录同时生成 `20260804_220000_batch.json`，记录字段顺序及会话路径；
- 任一字段发生持久错误，或用户点击停止，整批任务立即停止，不会跳过失败字段；
- 恢复旧的单字段会话时必须关闭多字段选项。

每个字段的变体都从同一份基础 DNA 生成，所以开始下一字段时，上一个字段会
自动恢复到基础值，不会把多个字段的变化叠加起来。
...
experiments/dna_field_sweeps/
  20260804_220000_batch.json       # 多字段批次清单
  20260804_220000_001_gene_chin_forward/
