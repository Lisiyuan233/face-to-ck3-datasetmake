...
默认配置：ConvNeXt-Tiny、ImageNet 预训练、`256×384`、几何/颜色双视图、全局 batch 32、BF16、30 epoch。显存不足时优先把 `batch_size` 调小并按比例增大 `gradient_accumulation`。
...
python train.py `
  ...
  --resume runs/convnext_tiny_dual_view/last.pt `
  --device cuda
```
...
默认写入 `runs/convnext_tiny_dual_view/`：
...
模型选择分数由 signed MAE、strength MAE、color MAE 和 categorical macro-F1 组成，越低越好。6 个单类别字段不建立分类头，但仍训练其强度。
...
```powershell
python predict.py `
  --checkpoint runs/convnext_tiny_dual_view/best.pt `
...
默认使用 checkpoint 中的 EMA 参数；添加 `--raw-weights` 可改用原始参数。输出前同样校验 schema SHA-256。
